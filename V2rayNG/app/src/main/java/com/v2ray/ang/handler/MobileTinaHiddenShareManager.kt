...
/**
 * The only configuration-output helper intentionally exposed by MobileTina.
 * It is called exclusively from the subscription panel revealed after a 10-second hold.
 */
object MobileTinaHiddenShareManager {

    fun copyAllConfigs(context: Context, subscriptionId: String): Int {
        val contents = MmkvManager.decodeServerList(subscriptionId)
            .mapNotNull { guid -> exportConfig(context, guid)?.trim()?.takeIf { it.isNotEmpty() } }

        if (contents.isEmpty()) return 0
        Utils.setClipboard(context, contents.joinToString(separator = "\n\n"))
        return contents.size
    }

    private fun exportConfig(context: Context, guid: String): String? {
        ...
        return when (profile.configType) {
            ...
            else -> CoreConfigManager.getV2rayConfig(context, guid)
                .takeIf { it.status }
                ?.content
        }
    }
